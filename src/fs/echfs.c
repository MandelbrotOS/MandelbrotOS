...
echfs_t echfs_get_fs(device_t device)
{
  ...
  echfs_t fs = (echfs_t) { table.block_cnt, table.dir_block_cnt,
    table.block_size, 0, 0, 0 };
    ...
    device_t device, echfs_t fs, uint64_t dir, const char *name)
{
  ...
  for (uint64_t i = 0; i < fs.dir_cnt; i++) {
    device.read(device.device,
        (void *)(fs.dir_offset + i * sizeof(echfs_entry_t)), (uint64_t)(&entry),
        sizeof(echfs_entry_t));
